...
function useRenderElementProps(componentProps, params = {}) {
  ...
  if (disableStyleHooks !== true) {
    styleHooks = React10.useMemo(() => enabled ? getStyleHookProps(state, customStyleHookMapping2) : EMPTY_OBJECT, [state, customStyleHookMapping2, enabled]);
  }
...
function evaluateRenderProp(element, render, props, state) {
  if (render) {
    ...
    return React10.cloneElement(render, mergedProps);
  }
  if (element) {
...
function renderTag(Tag, props) {
  ...
  return React10.createElement(Tag, props);
}
...
var NumberFieldRoot = React13.forwardRef(function NumberFieldRoot2(componentProps, forwardedRef) {
  const {
    id: idProp,
    min,
    max,
  ...
  const [isScrubbing, setIsScrubbing] = React13.useState(false);
  ...
  const isPressedRef = React13.useRef(false);
  const movesAfterTouchRef = React13.useRef(0);
  const allowInputSyncRef = React13.useRef(true);
  const unsubscribeFromGlobalContextMenuRef = React13.useRef(() => {
  });
  useIsoLayoutEffect(() => {
  ...
  const [inputMode, setInputMode] = React13.useState("numeric");
  ...
  React13.useEffect(function registerElementWheelListener() {
...
var NumberFieldGroup = React14.forwardRef(function NumberFieldGroup2(componentProps, forwardedRef) {
  const {
    render,
    className,
    ...elementProps
...
function useNumberFieldButton(params) {
  ...
  const incrementDownCoordsRef = React18.useRef({
    x: 0,
    y: 0
  });
  const isTouchingButtonRef = React18.useRef(false);
  const ignoreClickRef = React18.useRef(false);
...
// node_modules/@base-ui-components/react/esm/number-field/increment/NumberFieldIncrement.js
var NumberFieldIncrement = React19.forwardRef(function NumberFieldIncrement2(componentProps, forwardedRef) {
  const {
    render,
    className,
    disabled: disabledProp = false,
  ...
  const buttonState = React19.useMemo(() => ({
    ...state,
    disabled
  }), [state, disabled]);
...
var NumberFieldDecrement = React20.forwardRef(function NumberFieldDecrement2(componentProps, forwardedRef) {
  const {
    render,
    className,
    disabled: disabledProp = false,
  ...
  const buttonState = React20.useMemo(() => ({
    ...state,
    disabled
  }), [state, disabled]);
  const element = useRenderElement("button", componentProps, {
...
var FormContext = React21.createContext({
  formRef: {
    current: {
...
function useFieldControlValidation() {
  ...
  const getValidationProps = React22.useCallback((externalProps = {}) => mergeProps({
    ...messageIds.length && {
  ...
  const getInputValidationProps = React22.useCallback((externalProps = {}) => mergeProps({
    onChange(event) {
...
var NumberFieldInput = React23.forwardRef(function NumberFieldInput2(componentProps, forwardedRef) {
  const {
    render,
    className,
    ...elementProps
  ...
  const hasTouchedInputRef = React23.useRef(false);
  const blockRevalidationRef = React23.useRef(false);
  useField({
    id,
    commitValidation,
    value,
  ...
  const prevInputValueRef = React23.useRef(inputValue);
  useIsoLayoutEffect(() => {
...
var NumberFieldScrubAreaContext = React24.createContext(void 0);
if (true) NumberFieldScrubAreaContext.displayName = "NumberFieldScrubAreaContext";
function useNumberFieldScrubAreaContext() {
  const context = React24.useContext(NumberFieldScrubAreaContext);
  if (context === void 0) {
...
var NumberFieldScrubArea = React25.forwardRef(function NumberFieldScrubArea2(componentProps, forwardedRef) {
  const {
    render,
    className,
  ...
  const scrubAreaRef = React25.useRef(null);
  const isScrubbingRef = React25.useRef(false);
  const scrubAreaCursorRef = React25.useRef(null);
  const virtualCursorCoords = React25.useRef({
    x: 0,
    y: 0
  });
  const visualScaleRef = React25.useRef(1);
  const [isTouchInput, setIsTouchInput] = React25.useState(false);
  const [isPointerLockDenied, setIsPointerLockDenied] = React25.useState(false);
  React25.useEffect(() => {
  ...
  const onScrub = React25.useCallback(({
    movementX,
    movementY
  }) => {
  ...
  const onScrubbingChange = React25.useCallback((scrubbingValue, {
    clientX,
    clientY
  }) => {
    ReactDOM2.flushSync(() => {
  ...
  React25.useEffect(function registerGlobalScrubbingEventListeners() {
  ...
  React25.useEffect(function registerScrubberTouchPreventListener() {
...
var NumberFieldScrubAreaCursor = React26.forwardRef(function NumberFieldScrubAreaCursor2(componentProps, forwardedRef) {
  const {
    render,
    className,
    ...elementProps
  ...
  const [domElement, setDomElement] = React26.useState(null);
